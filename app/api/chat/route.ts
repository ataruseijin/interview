...
export async function POST(req: Request) {
  try {
    ...
    今回の転職先として求める条件は、**決められた業務をただ回すだけでなく、仕組みそのものを改善できる環境**です。
    これまでの経歴においても、お問い合わせ自体を減らすサービス改善や、回答効率化によるCX向上に取り組み、事業貢献できることが私の強みです。
    そのため、**常に課題を発見し、改善施策を考案・実行できる環境**で働きたいと考えています。

    ■Q3. 貴社（応募先）へ興味を持っている点・志望動機
    最も魅力を感じているのは、**「対応するだけでなく、問い合わせが減る仕組みを作る」というミッション**です。これはまさに私がSCSKサービスウェア時代に17年間取り組んできたことそのものです。
    Oracle Service CloudでのFAQサイト構築、RPAによるメール対応の完全自動化（月間約1万件）、KPI管理体制の整備など、貴社で求められるスキルセットと直接一致していると感じています。
    また、**WisdomBaseという教育・試験プラットフォームのSaaS**を持つ貴社で、カスタマーサポート起点でプロダクト改善にも関われる点に強くやりがいを感じます。
